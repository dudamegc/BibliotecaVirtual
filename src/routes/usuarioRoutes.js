...
router.post("/usuarios", async (req, res, next) => {
  try {
    ...
    const usuario = await Usuario.create({ nome, email, senha, tipo: tipo || "leitor" });
...
// Recuperar senha (só envia mensagem, pode depois gerar token de recuperação)
router.post("/recuperar-senha", async (req, res, next) => {
  try {
    const { email } = req.body;
    const usuario = await Usuario.findOne({ email });
    if (!usuario) return res.status(404).json({ error: "E-mail não encontrado." });
    res.json({ mensagem: "Instruções de recuperação enviadas para o e-mail." });
  } catch (error) {
    next(error);
  }
});
